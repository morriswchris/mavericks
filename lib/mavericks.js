...
function initialize(opts) {
  ...
  app.use(express.static(path.join(__dirname, options.staticPath)));
  ...
  app.put("/:collection/:id", function(req, res, next) {
    req.collection.update({"_id": req.params.id}, {$set: req.body}, {multi: false}, function(e, status) {
      if (e) {
        return next(e);
      }
  ...
  // Save our mongoose connect to app for reuse
  app.db = db;
  return app;
}
